...
namespace Backend_Projet_BDD.Service
{
    public class LogementService: ILogementService
    {
        ...
        {
              String query = "Select l.*, p.nom as 'nomProprio', count(distinct g.id_Garage) as nbGarages,ifnull((select (c.commissionBase + CAST(c.pourcentage as DECIMAL)/100 * l.prix) as commission from Commission c where c.id_Logement = l.id_Logement order by c.date_Commission desc LIMIT 1),0) as commission, max(v.date_Visite) as lastVisite from Logement l inner join Personne p on p.id_Personne= l.id_Proprietaire inner join Garage g on l.id_Logement = g.id_Logement LEFT join Commission c on c.id_Logement = l.id_Logement LEFT join Visite v on v.id_Logement = l.id_Logement group by l.id_Logement,l.adresse,l.type,l.nbPiece,l.surface,l.etat,l.objetGestion,l.prix,l.dateDispo,l.ville,l.id_Proprietaire, p.nom;";
        ...
        public List<Logement> getLogementByCriteria(Criteria criteria)
        {
            ...
            String querytmp = "Select l.*, p.nom as 'nomProprio', count(distinct g.id_Garage) as nbGarages,ifnull((select (c.commissionBase + CAST(c.pourcentage as DECIMAL)/100 * l.prix) as commission from Commission c where c.id_Logement = l.id_Logement order by c.date_Commission desc LIMIT 1),0) as 'commission', max(v.date_Visite) as lastVisite from Logement l inner join Personne p on p.id_Personne= l.id_Proprietaire inner join Garage g on l.id_Logement = g.id_Logement LEFT join Commission c on c.id_Logement = l.id_Logement LEFT join Visite v on v.id_Logement = l.id_Logement " + whereTotal;
